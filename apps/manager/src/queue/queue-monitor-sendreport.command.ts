...
export default (config: Config, logger: Logger) => {
  return command({
    name: 'queue-monitor-sendreport',
    description: 'Queue monitor sendreport',
    args: {},
    handler: async () => {
      const queueConnection = queueService.getQueueConnection<Config>(config)
      const queueEvents = queueService.initQueueEvents(queueConnection, SENDREPORT_QUEUE_NAME, logger)
